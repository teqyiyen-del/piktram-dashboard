...
type SidebarProps = {
  role?: 'admin' | 'user'
  open?: boolean        // sadece mobil için
  onClose?: () => void  // sadece mobil için
}
...
export default function Sidebar({ role = 'user', open = false, onClose }: SidebarProps) {
          ...
          return (
            <li key={item.href}>
              ...
              >
                <span
                  className={cn(
                    'flex h-8 w-8 min-w-[2rem] items-center justify-center rounded-lg transition-colors',
                    isActive
  ...
  return (
    <>
      {/* ===== Desktop: SABİT, tam ekran yüksekliği ===== */}
      <aside
        className="hidden lg:flex lg:flex-col fixed top-0 left-0 h-screen w-[280px] shrink-0 border-r border-gray-200/70
                   bg-gradient-to-b from-white via-white to-[#FFF5F3] px-6 py-6 shadow-xl transition-colors duration-300
                   dark:border-gray-800/70 dark:from-[#171717] dark:via-[#171717] dark:to-[#151515] z-40"
      >
        {/* Logo / Üst */}
        ...
        <div className="flex flex-1 flex-col min-h-0">
          {renderNav()}
          <div className="mt-4">
            <div className="rounded-xl bg-white/80 p-4 text-xs text-gray-600 shadow-sm backdrop-blur dark:bg-surface-dark/80 dark:text-gray-300">
      ...
      {/* ===== Mobile: overlay (open/onClose burada geçerli) ===== */}
