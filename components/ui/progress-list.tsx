  ...
  return (
    <div className="space-y-5">
      {items.map((item) => {
        ...
        return (
          <div key={item.title} className="space-y-2 rounded-2xl border border-gray-200 bg-white/90 p-4 shadow-sm transition-colors duration-300 dark:border-gray-700 dark:bg-gray-900/60">
            ...
            {item.targetLabel ? (
              <p className="text-xs text-gray-500 dark:text-gray-400">{item.targetLabel}</p>
            ) : null}
          </div>
        )
      })}
